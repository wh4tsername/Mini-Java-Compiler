...
void PrintVisitor::Visit(JumpConditionalStatement* jump_conditional_statement) {
  ...
  PrintTabs();
  stream_ << "TrueLabel: " << jump_conditional_statement->label_true_.ToString()
          << std::endl;
  PrintTabs();
  stream_ << "FalseLabel: "
...
void PrintVisitor::Visit(LabelStatement* label_statement) {
  PrintTabs();
  stream_ << "LabelStatement: " << label_statement->label_.ToString()
          << std::endl;
}

void PrintVisitor::Visit(BinopExpression* binop_statement) {
  PrintTabs();
  stream_ << "BinopExpression: " << ToString(binop_statement->type_)
          << std::endl;
...
void PrintVisitor::Visit(TempExpression* temp_exression) {
  PrintTabs();
  stream_ << "TempExpression: " << temp_exression->temporary_.ToString()
          << std::endl;
}
...
void PrintVisitor::Visit(JumpStatement* jump_statement) {
  PrintTabs();
  stream_ << "JumpStatement: " << jump_statement->label_.ToString()
          << std::endl;
}
...
void PrintVisitor::Visit(NameExpression* name_expression) {
  PrintTabs();
  stream_ << "NameExpression: " << name_expression->label_.ToString()
          << std::endl;
}
